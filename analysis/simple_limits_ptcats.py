...
if __name__ == "__main__":
	...
	if args.make_datacards:
		...
		datacard_dir = "/uscms/home/dryu/HbbPlusJet/data/datacards/simple_ptcats"
		...
		region_containers = {}
		for pt_index in xrange(len(default_pt_categories)):
			region_name = "ptcat{}".format(pt_index + 1)
			...
			region_containers[region_name].add_data("data_obs", h2d_slices["data_obs"][pt_index])
			# def add_signal(self, signal_name, pass_hist, fail_hist, normalization_var=None):

			for signal in signals:
				region_containers[region_name].add_signal(signal, h2d_slices[signal][pt_index])

			for background_name in backgrounds:
				region_containers[region_name].add_background(background_name, h2d_slices[background_name][pt_index])
			...
			for process in backgrounds + signals:
				...
				bbsf = 0.91
				dbbsf = 0.03
				region_containers[region_name].add_norm_systematic("beff", process, unc_value=1 + dbbsf)
			...
			for process in ["wqq", "zqq"]:
				...
				region_containers[region_name].add_norm_systematic("znormEW", process, unc_value=1.15)
			...
			for systematic in ["JER", "JES", "Pu", "trigger"]:
				for process in backgrounds + signals:
					...
					region_containers[region_name].add_norm_systematic(systematic, process, unc_value=syst_value)
			...
			#	region_containers[region_name].add_shape_systematic("massscale", process, hists_massscale_pass, hists_massscale_fail)
			...
			#	region_containers[region_name].add_shape_systematic("masssmear", process, hists_masssmear_pass, hists_masssmear_fail)
